...
function makeResult(input: Input): Result {
  return {
    ...
    tablesSeasons: [],
    tablesAllVsAll: [],
    tableRanking: Array(input.teams.length).fill(-1).map((_, i) => i),
    tablePoints: Array(input.teams.length).fill(null).map(() => ({ total: 0 })),
  };
}

export function generate(input: Input): Result {
  return update(makeResult(input));
}
...
export function update(resultIn: Result): Result {
  ...
  Object.keys(result.allMatches) // eslint-disable-line flowtype-errors/show-errors
    .forEach((indexId: number) => {
      ...
      if (match.result.team1Score && match.result.team2Score) {
        ...
        const winnerTeamIndex: number = match.result.winnerTeamIndex;
        if (winnerTeamIndex === match.team1Index) {
          team1Points.total += 3;
        } else if (winnerTeamIndex === match.team2Index) {
          team2Points.total += 3;
        } else {
          team1Points.total += 1;
          team2Points.total += 1;
  ...
  result.tableRanking.sort((teamIndex1: number, teamIndex2: number) =>
    result.tablePoints[teamIndex2].total - result.tablePoints[teamIndex1].total,
  );
